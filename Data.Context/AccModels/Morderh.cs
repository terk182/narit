using System;
using System.Collections.Generic;

namespace narit_mis_api.AccModels
{
    public partial class Morderh
    {
        public string Fcdataser { get; set; } = null!;
        public string Fcskid { get; set; } = null!;
        public string Fcudate { get; set; } = null!;
        public string Fcutime { get; set; } = null!;
        public string Fclupdapp { get; set; } = null!;
        public string Fcrftype { get; set; } = null!;
        public string Fcreftype { get; set; } = null!;
        public string Fccorp { get; set; } = null!;
        public string Fcbranch { get; set; } = null!;
        public string Fcplant { get; set; } = null!;
        public string Fcsubbr { get; set; } = null!;
        public string Fcdept { get; set; } = null!;
        public string Fcsect { get; set; } = null!;
        public string Fcproj { get; set; } = null!;
        public string Fcjob { get; set; } = null!;
        public string Fcgvpolicy { get; set; } = null!;
        public string Fcpriority { get; set; } = null!;
        public string Fcstat { get; set; } = null!;
        public string Fcstep { get; set; } = null!;
        public string Fcstep2 { get; set; } = null!;
        public DateTime Fddate { get; set; }
        public DateTime? Fdstartdat { get; set; }
        public DateTime? Fdduedate { get; set; }
        public DateTime? Fdrecedate { get; set; }
        public DateTime? Fdrelease { get; set; }
        public DateTime? Fdfinish { get; set; }
        public DateTime? Fdetddate { get; set; }
        public DateTime? Fdeffectiv { get; set; }
        public DateTime? Fdlbatrun { get; set; }
        public string Fcmaster { get; set; } = null!;
        public string Fcparent { get; set; } = null!;
        public string Fcmbook { get; set; } = null!;
        public string Fccode { get; set; } = null!;
        public string Fcrefno { get; set; } = null!;
        public string Fcpdsth { get; set; } = null!;
        public string Fcmainprod { get; set; } = null!;
        public string Fclevel { get; set; } = null!;
        public string Fcmoset { get; set; } = null!;
        public decimal Fnstdtime { get; set; }
        public string Fccoor { get; set; } = null!;
        public string Fcempl { get; set; } = null!;
        public decimal Fnmosize { get; set; }
        public decimal Fnbatchsiz { get; set; }
        public decimal Fnbatchint { get; set; }
        public string Fccreateby { get; set; } = null!;
        public string Fccorrectb { get; set; } = null!;
        public string Fccancelby { get; set; } = null!;
        public string Fcapproveb { get; set; } = null!;
        public string? Fmmemdata { get; set; }
        public string Fccreatety { get; set; } = null!;
        public string Fceafterr { get; set; } = null!;
        public string Fcseltag { get; set; } = null!;
        public DateTime Ftdatetime { get; set; }
        public int Fimillisec { get; set; }
        public string Fcmps { get; set; } = null!;
        public string Fcplanmoh { get; set; } = null!;
        public string Fcbtype { get; set; } = null!;
        public string Fchold { get; set; } = null!;
        public string Fcsorefno { get; set; } = null!;
        public DateTime? Fdsodate { get; set; }
        public string Fcqcbookso { get; set; } = null!;
        public string Fcqcso { get; set; } = null!;
        public string Fcgmachine { get; set; } = null!;
        public string Fcmachine { get; set; } = null!;
        public string Fcshift { get; set; } = null!;
        public string Fctxid { get; set; } = null!;
        public string Fcsubtxid { get; set; } = null!;
        public string Fcbomseq { get; set; } = null!;
        public DateTime? Ftlastupd { get; set; }
        public DateTime? Fdexplode { get; set; }
        public decimal Fnmostsize { get; set; }
        public decimal Fnmomfsize { get; set; }
        public DateTime? Ftrelease { get; set; }
        public string Fcmplanh { get; set; } = null!;
        public string Fccum { get; set; } = null!;
        public decimal Fncumqty { get; set; }
        public decimal Fnplanqty { get; set; }
        public string Fcmpsd1 { get; set; } = null!;
        public string Fcgentab { get; set; } = null!;
        public string Fcgenskid { get; set; } = null!;
        public string? Fmmemdata2 { get; set; }
        public string? Fmmemdata3 { get; set; }
        public string? Fmmemdata4 { get; set; }
        public string? Fmmemdata5 { get; set; }
        public string Fcbatchno { get; set; } = null!;
        public DateTime? Ftlastedit { get; set; }
        public string Fccreateap { get; set; } = null!;
        public string Fcu1acc { get; set; } = null!;
        public string Fcdataimp { get; set; } = null!;
        public DateTime? Fddelivery { get; set; }
        public string Fcgenby { get; set; } = null!;
        public string Fctoolmold { get; set; } = null!;
        public string Fcgcode { get; set; } = null!;
        public DateTime? Fdgdate { get; set; }
        public string Fcsetcode { get; set; } = null!;
        public string Fcdrawpic { get; set; } = null!;
        public string Fclid { get; set; } = null!;
        public string Fcdocflowi { get; set; } = null!;
        public string? Fmdocflow { get; set; }
        public string Fcboicard { get; set; } = null!;
        public string Fcboigroup { get; set; } = null!;
    }
}
