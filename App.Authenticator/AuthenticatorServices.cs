...
namespace App.Authenticator
{
    public class AuthenticatorServices : IAuthenticatorServices
    {
        ...
        public AuthenticatorBaseResponse StaffSetup(UserStaffRequest request)
        {
            var _staff = new Staff();
            _staff.Id = request.Id;
            _staff.Name = request.Name;
            _staff.Surname = request.Surname;
            //_staff.HrdepartmentId = request.HrdepartmentId;
            //_staff.JobPositionId = request.JobPositionId;
            //_staff.MobilePhoneNumber = request.MobilePhoneNumber;

            if (request.EMailAddress != null)
            {
                _staff.EMailAddress = request.EMailAddress;
            }

            _database.Entry(_staff).State = _staff.Id == 0 ?
                                               EntityState.Added :
            ...
            var response = new AuthenticatorBaseResponse();
            response.Success = result > 0 ? true : false;
            response.Messsage = _staff.Id == 0 ? "update" : "insert";
            return response;
