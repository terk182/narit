    ...
    public class SecServices : ISecServices
    {
            ...
            foreach (var s1 in planActivity)
            {
                ...
                foreach (var item in PlanItems)
                {
                    ...
                    _list.Add(new ViewPlanForActivityByPlanTypeBudgetTypeTable
                    {
                        ...
                        Unit = item.Unit,
                        BudgetTypeId = item.BudgetTypeId,
                        TotalBudgetCache = item.SummaryStatementCaches.Count > 0 ? item.SummaryStatementCaches.Where(x => x.Active && x.Month == Month).SingleOrDefault().TotalBudgetPlanView : 0,
                        NetBudgetCache = item.SummaryStatementCaches.Count > 0 ? item.SummaryStatementCaches.Where(x => x.Active && x.Month == Month).SingleOrDefault().NetBudgetPlanView : 0,
                        UsedBudgetCache = item.SummaryStatementCaches.Count > 0 ? item.SummaryStatementCaches.Where(x => x.Active && x.Month == Month).SingleOrDefault().UsedBudgetPlanView : 0,
                        RemainBudgetCache = item.SummaryStatementCaches.Count > 0 ? item.SummaryStatementCaches.Where(x => x.Active && x.Month == Month).SingleOrDefault().RemainBudgetPlanView : 0,
                    });
                    b_num.Add((int)item.BudgetTypeId);
                    b_list.Add(new butgetW
                    {
        ...
        public List<PlanCoreGetAllbyStaffIdDto> GetAllPlanCorebyStaffId(int FiscalYear, int StaffId)
        {
            ...
            foreach (var item in responsibleperson)
            {
                ...
                var plantype = _database.PlanTypes.ToList();
                var QSOuterJoin = from user in data
                                  join Plantype in plantype
                                  on user.PlanTypeId equals Plantype.Id
                                  into UserPlantypeGroup
                                  from plantypeuser in UserPlantypeGroup.DefaultIfEmpty()
                                  select new {user, plantypeuser };
                foreach (var item2 in QSOuterJoin)
                {
                    result.Add(new PlanCoreGetAllbyStaffIdDto
                                    {
                                        Id = item2.user.Id,
                                        Name = item2.user.Name,
                                        PlanTypeId = item2.user.PlanTypeId,
                                        PlanTypeName = item2.plantypeuser.Name
                    });
                }
        ...
        //public List<PlanCoreGetAllbyStaffIdDto> GetAllPlanCorebyStaffId(int fisicalYear, int staffId)
        //{
        //    var responsibleperson = _database.ResponsiblePersons.Where(x => x.FiscalYear == fisicalYear && x.StaffId == staffId).ToList();
        //    var result = new List<PlanCoreGetAllbyStaffIdDto>();
        //    foreach (var item in responsibleperson)
        //    {
                //var data = _database.PlanCores.Where(x => x.FiscalYear == fisicalYear && x.Id == item.PlanCoreId && x.Active == true).ToList();
